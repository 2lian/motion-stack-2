    ...
    for jsl in state_orderinator_3000(states):
        l += [
            cycl_msgs.JointState(
                ...
                name=[js.name.encode("utf-8") for js in jsl],
                position=[js.position for js in jsl],
                velocity=[js.velocity for js in jsl],
                effort=[js.effort for js in jsl],
            )
        ]
    return l
    ...
    for jsl in state_orderinator_3000(states):
        l += [
            cydr_msgs.JointState(
                ...
                position=np.array([js.position for js in jsl], dtype=np.float64),
                velocity=np.array([js.velocity for js in jsl], dtype=np.float64),
                effort=np.array([js.effort for js in jsl], dtype=np.float64),
            )
        ]
    return l
